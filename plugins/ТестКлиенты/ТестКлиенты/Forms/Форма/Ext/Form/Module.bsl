...
Функция ПортПоУмолчанию() Экспорт
	Если Не ЗначениеЗаполнено(ПортПоУмолчанию) Тогда
		УстановитьПортПоУмолчанию(48000);
	КонецЕсли;
...
Функция СтрокаЗапускаТестКлиента(Знач ИмяПользователя, Знач Пароль, Знач Порт, Знач ДопПараметры)
	...
	Если ЗначениеЗаполнено(ИмяПользователя) Тогда
		ОтключитьДиалогАутентификации = "/DisableStartupDialogs";
	КонецЕсли;
	...
	Результат = КонтекстЯдра.СтрШаблон_(
		"""%1"" ENTERPRISE /IBConnectionString""%2""%3%4 /TestClient -TPort%5 /L%6 %7 /DisableStartupMessages %8",
		СтрокаЗапуска1с,
